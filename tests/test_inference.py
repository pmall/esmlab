...
from esmlab.storage import FileLogitsStorage, logits_key
...
def _settings(
    ...
    The stub needs no credentials, so the backend fields are blanked. Storage
    and the perf CSV both live under ``tmp_path`` so tests never touch the real
    ``data/`` tree.
    """
...
def test_second_identical_run_skips_everything(tmp_path: Path) -> None:
    ...
    model_dir = settings.storage_root / "esmc-600m"
    assert [path.name for path in model_dir.iterdir()] == [logits_key(SEQUENCE)]
...
def test_perf_csv_gets_a_header_and_one_row_per_run(tmp_path: Path) -> None:
    ...
    assert rows[0]["storage_root"] == str(settings.storage_root)
